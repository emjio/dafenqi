...
import { Key, Bot, Cpu, ShieldCheck, Sparkles, WifiOff } from 'lucide-react';
...
export const SetupScreen: React.FC<SetupScreenProps> = ({ onStart }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (provider !== 'algorithm' && !apiKey.trim()) {
      setError('请输入 API Key 以继续');
      return;
    }
    
    onStart(provider, provider === 'algorithm' ? 'dummy-key' : apiKey);
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            <label className="text-slate-300 text-sm font-bold flex items-center gap-2">
              <Cpu size={16} className="text-indigo-400" /> 选择对手类型
            </label>
            <div className="grid grid-cols-1 gap-3">
              ...
              <button
                type="button"
                onClick={() => setProvider('algorithm')}
                className={`
                  flex items-center justify-center gap-2 py-3 px-4 rounded-xl border-2 transition-all
                  ${provider === 'algorithm' 
                    ? 'bg-slate-700 border-slate-400 text-white shadow-lg' 
                    : 'bg-slate-800 border-slate-700 text-slate-500 hover:border-slate-600 hover:text-slate-300'}
                `}
              >
                <WifiOff size={20} />
                <span>我没有 API Key (挑战纯算法)</span>
              </button>
            </div>
          </div>

          {/* API Key Input (Hidden for Algorithm) */}
          {provider !== 'algorithm' && (
          ...
          >
            {provider === 'algorithm' ? '启动算法引擎' : '初始化系统并进入'}
          </button>
        </form>
      </div>
    </div>
